# Xiaomi MiPad 2 - Установка Windows 10 с DualBoot и без
<img width="293" height="495" alt="PXtiYkk4Y9IIfPubPUA6DSUKEe0BcLTV4Tb" src="https://github.com/user-attachments/assets/94fc125f-aa70-4511-8031-e7fabd4b2467" />
                                    
# Вам понадобится:

* MiPad2 (16 ГБ или 64 ГБ)
  
* Загрузочная флешка, USB-Hub, USB-мышь и клавиатура
  
* ISO-файл Windows 10

* Записать ISO-образ можно через:
  
* [UltraISO](https://www.ezbsystems.com)
  
* [Rufus](https://rufus.ie)
  
* Если у вас версия на 16 ГБ - Не рекомендуется ставить DualBoot(Windows и Android)
  
* Драйверы устройств для Windows 10 (включены в этот репозиторий)
  
* Double Driver (найдите его в Google или загрузите из этого репозитория)

* [7Zip](https://www.7-zip.org) или [WinZip](https://www.winzip.com) 


# Шаг 1 - Загрузите Windows 10:                           

Вариант 1: Загрузите программу восстановления Xiaomi для Windows 10 (20160125-10586-oobe-16G.zip) с [Archive.org](https://archive.org/download/fr.twb.s_xiaomi_mipad2_factory) 

Возможно, вам придется это сделать, если у вас установлен MIUI

Внимание: если у вас 16-гигабайтный MiPad2, у вас останется не так много места (возможно, 6-8 ГБ) на этом Win10                    

Вариант 2: Скачайте Windows 10 Home 1607 64 bit:

* Перейдите по ссылкам в описании:
                                         
* [Сборка](https://archive.org/details/win10-1607)


Вариант 3: Скачайте NTLite и переупакуйте Win 10 самостоятельно

# Шаг 2 - Установите Windows 10 на USB-накопитель:

* Запишите образ через UltraISO или Rufus
             
* UltraISO Выберите файл открыть → Windows 10 Home 1607 64 bit → Самозагрузка → Записать образ жёсткого диска → Выберите флешку → Записать
                                 
* Rufus Метод загрузки → Выбрать → Windows 10 Home 1607 64 bit → Выберите устройство → Старт                     
                                                               
                                
# Шаг 3 - Установите драйвер Double Driver на USB-накопитель: 

* Если вы еще этого не сделали, загрузите драйвер Double Driver из этого репозитория, извлеките .zip и поместите папку "Double Driver" на свой USB-накопитель

* Аналогичный способ загрузите папку Double Driver с [Google Drive](https://drive.google.com/drive/folders/1NXDX2EymJNuaUzyirQfSixEYFGPG0MFy) и распакуйте .zip
                    
                                               
# Шаг 4 - Установите драйверы устройств Windows 10 на USB-накопитель:

* Если вы еще этого не сделали, загрузите файлы драйверов устройств для Windows 10 из этого репозитория
  
* Вам нужно будет использовать 7Zip или WinRAR для извлечения всех файлов драйверов
  
* Распакуйте архив "win10_drivers.7zip" на свой USB-накопитель

* Аналогичный способ загрузите папку Drivers с [Google Drive](https://drive.google.com/drive/folders/1NXDX2EymJNuaUzyirQfSixEYFGPG0MFy) и распакуйте .zip
               

# Шаг 5 - Установка Windows:

* Подключите USB-накопитель, USB-мышь и клавиатуру к USB-Hub. Подключите USB-Hub к MiPad 2
  
* Включите планшет, и нажимайте F2 на клавиатуре → Boot Maintenance Manager → Boot From File → Efi File(нужно найти и выбрать)
  
* Продолжайте установку Windows 10: Далее → Установить → У меня нет ключа продукта → Выбрать редакцию → Далее →  Пользовательское соглашение → Удаляете все Android разделы → Выбираете пустой диск → После того как установка закончится во время перезагрузки выньте флешку
* После перезагрузки выполните необходимые настройки, которые предложит операционная система Windows

# Шаг 6 - Установите драйверы Xiaomi для Windows:
                                       
* Как только Windows 10 будет установлена, перейдите на свой USB-накопитель через проводник, выберите "Двойной драйвер", выполните dd.exe

* Нажмите "Restore → Locate Backup → Other Location". Найдите драйверы устройств Windows 10, которые вы извлекли на шаге 4

* Запустите процесс восстановления драйвера

* Если у вас не получается поставить через Double Driver, вы можете использовать [Driver Booster](https://ru.iobit.com/driver-booster-pro.php) и [Snappy Driver Installer](https://sdi-tool.org)
                              
# Шаг 7 - Исправьте нерабочий Wi-Fi:
                                      
* Перейдите в Диспетчер устройств, найдите Сетевые адаптеры -> Broadcom 802.11ac Wireless PCIE Full Dongle Adapter                              
                    
* Перейдите в Свойства
                                
* Перейдите на вкладку Драйверы, нажмите Обновить драйвер. Выберите Поиск драйверов на моем компьютере                          

* Выберите "Позвольте мне выбрать из списка доступных драйверов на моем компьютере"

* Вместо использования драйвера Microsoft выберите драйвер Broadcom из распакованного архива win10_drivers.rar

# Шаг 8 - Исправление нерабочей камеры:

## Нам нужно отключить безопасную загрузку, поскольку драйвер камеры не был подписан должным образом
Откройте CMD от имени администратора

* Введите команду:

```
bcdedit /set loadoptions DDISABLE_INTEGRITY_CHECKS
```

* Закройте командную строку

* Теперь перейдем к последнему шагу (чтобы отключить принудительное использование подписи драйвера)

* Это самый простой способ отключить принудительное использование подписи драйвера в Windows 10, но имейте в виду, что этот метод отключает подпись драйвера только временно

* Чтобы отключить принудительное использование подписи драйвера, выполните следующие действия:

* Нажмите и удерживайте клавишу Shift на клавиатуре и нажмите кнопку перезапуск

* Выберите "Устранение неполадок" → "Дополнительные параметры" → "Параметры запуска" и нажмите кнопку перезапуск
  
* Когда компьютер перезагрузится, вы увидите список опций. Нажмите F7 или 7 на клавиатуре, чтобы выбрать "Отключить принудительное использование подписи драйвера"

* Теперь ваш компьютер перезагрузится, и вы сможете установить неподписанные драйверы

* Имейте в виду, что этот метод только временно отключает принудительное использование подписи драйверов, поэтому обязательно установите все неподписанные драйверы как можно скорее

* Установите драйвер камеры с помощью двойного драйвера (win10_drivers\Image\Intel(R) AVStream Camera)

* Тема на [4PDA](https://4pda.to/forum/index.php?showtopic=785498&st=1420)
